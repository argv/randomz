...
class syntax_plugin_randomz extends DokuWiki_Syntax_Plugin {
    ...
    function connectTo($mode) {
        $this->Lexer->addSpecialPattern('\{\{randomz>[^}]*\}\}',$mode,'plugin_randomz');
    }
    ...
    /**
     * Does the randomz formatting
     */
    function _randomz($data){
        global $conf;
        global $lang;
        $ret = '';
        ...
        if($data['align'] == 3){
            $align  = ' randomz_center';
            $xalign = ' align="center"';
        }
        if(!$data['_single']){
            if(!$align) $align = ' randomz_center'; // center galleries on default
        ...
        if($data['_single']){
            ...
            foreach($files as $img){
                ...
                if($data['paginate'] && ($i % $data['paginate'] == 0)){
                     $ret .= '<div class="randomz_page randomz__'.$data['galid'].'" id="randomz__'.$data['galid'].'_'.(++$page).'">';
                     $close_pg = true;
                }
        ...
        $pgret = '';
        if($page){
            $pgret .= '<div class="randomz_pages"><span>'.$this->getLang('pages').' </span>';
            for($j=1; $j<=$page; $j++){
                $pgret .= '<a href="#randomz__'.$data['galid'].'_'.$j.'" class="randomz_pgsel button">'.$j.'</a> ';
            }
            $pgret .= '</div>';
        }

        return '<div class="randomz'.$align.'"'.$xalign.'>'.$pgret.$ret.'<div class="clearer"></div></div>';
    }
